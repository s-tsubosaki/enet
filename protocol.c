...
static int
enet_protocol_receive_incoming_commands2(ENetHost *host, ENetEvent *events, enet_uint32 eventNum)
{
  if (eventNum > ENET_EVENT_RECV_MAX)
  {
    printf("eventNum is over %d\n", ENET_EVENT_RECV_MAX);
    return -1;
  }

  static enet_uint8 rbuffs[ENET_EVENT_RECV_MAX][4096];
  ENetBuffer buffers[ENET_EVENT_RECV_MAX];
  ENetAddress addresses[ENET_EVENT_RECV_MAX];

  ENetEvent *event = &events[0];
  if (host->intercept != NULL)
  {
    switch (host->intercept(host, event))
    {
  ...
  {
    buffers[i].data = rbuffs[i];
    buffers[i].dataLength = sizeof(rbuffs[i]);
  }

  int recvCount = enet_socket_receive2(host->socket, addresses, buffers, eventNum);
  for (int i = 0; i < recvCount; i++)
  {
    host->receivedData = buffers[i].data;
    host->receivedDataLength = buffers[i].dataLength;

    host->totalReceivedData += buffers[i].dataLength;
    host->totalReceivedPackets++;

    host->receivedAddress = addresses[i];

    int ret = enet_protocol_handle_incoming_commands(host, event);
    if (ret == 1)
    {
      ++event;
    }
  }
  return recvCount > 0 ? 1 : -1;
}
...
int enet_host_service2(ENetHost *host, ENetEvent *events, enet_uint32 eventNum, enet_uint32 timeout)
{
  enet_uint32 waitCondition;
  ENetEvent *event = &events[0];
  ...
  if (events != NULL)
  {
    for (int i = 0; i < eventNum; i++)
    {
      events[i].type = ENET_EVENT_TYPE_NONE;
      events[i].peer = NULL;
      events[i].packet = NULL;
    }
  ...
  do
  {
    ...
    switch (enet_protocol_receive_incoming_commands2(host, events, eventNum))
    {
    case 1:
      return 1;
